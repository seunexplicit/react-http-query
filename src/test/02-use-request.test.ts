import { act, BAD_URL, GOOD_URL, mockWindowProperty, renderHook, setupMockupRequest, __BAD_RESPONSE__, __MOCK_DATA__ } from "./test-util";
...
describe(`useRequest`, () => {
    ...
    test('should use url passed to makeRequest if absolute url', async () => {
        const { result: { current: [, makeRequest] } } = renderHook(() => useRequest({ baseUrl: 'http://baseUrl/' }));
        await  act(() => makeRequest(GOOD_URL));
    ...
    test(`should use baseUrl passed to useRequest if url passed to makeRequest is absolute url 
        ...
        const { result: { current: [, makeRequest] } } = renderHook(() => useRequest({ baseUrl }));
